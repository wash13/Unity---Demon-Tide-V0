...
namespace Gamekit3D
{
    public class MeleeWeapon : MonoBehaviour
    {
        ...
        private bool CheckDamage(Collider other, AttackPoint pts)
        {
            ...
            //if the target is immune from this angle, or is invulnerable, invoke on invulnerable hit
            if (d.isInvulnerable || !d.CheckAngle(data))
            {
                onHitInvulnerable.Invoke(); //triggers script for hitting invulnerable
            }


            d.ApplyDamage(data);

            if (!throwingHit) success = true;
