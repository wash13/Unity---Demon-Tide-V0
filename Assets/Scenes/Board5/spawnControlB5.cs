...
namespace Gamekit3D
{
    public class spawnControlB5 : MonoBehaviour
    {
        ...
        public IEnumerator spawn()
        {
            while (working)
            {
                ...
                StartCoroutine(workers[Random.Range(0, workers.Length)].spawnGroup(shadowDemon, 4, 6f, .5f));
                StartCoroutine(workers[Random.Range(0, workers.Length)].spawnGroup(hellBones, 1, 6f, .5f));
                StartCoroutine(workers[Random.Range(0, workers.Length)].spawnGroup(hellBones, 1, 6f, .5f));
                StartCoroutine(workers[Random.Range(0, workers.Length)].spawnGroup(scytheDemon, 2, 4f, .5f));
                StartCoroutine(workers[Random.Range(0, workers.Length)].spawnGroup(scytheDemon, 1, 4f, .5f));
